...
-c dna_r9.4.1_450bps_hac.cfg -i "$FAST5" -s "$LIBRARY_PATH"/"$LIBRARY_NAME"_ANALYSIS/BASECALL \
...
for i in $(cat "$CSV"); do
    ...
    --read-file "$LIBRARY_PATH"/"$LIBRARY_NAME"_ANALYSIS/ASSEMBLY/"$BARCODE"_"$LIBRARY_NAME".fastq \
